...
export default function EmployerSignUpForm({ onSuccess }: EmployerSignUpFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Sign up with employer type
      const { error } = await signUp(formData.email, formData.password, 'employer')
      
      if (error) {
        setError(error.message)
      } else {
        setMessage('Account created successfully! Please check your email for verification.')
        // Store additional employer data in localStorage for profile creation
        localStorage.setItem('pending_employer_data', JSON.stringify({
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { error } = await signInWithGoogle('/employers/new')
